...
static	void	ft_set_img(t_all *all, t_win *win)
{
	...
	if (win->no_addr == 0 || win->so_addr == 0)
		ft_perror("Error\n no no addr or so addr\n", all);
	ft_set_img2(all, win);
}

static	void	main2(char *argv, t_all *all)
{
	ft_check_argv2(argv, all);
	ft_screen_shot(all);
}
...
int				main(int argc, char **argv)
{
	...
	ft_check_argv1(argv[1], &all);
	if (argc > 1 && argc < 4)
		all.parcer_map = ft_read_map(argv[1], &all);
	else
		ft_perror("Error\n amount of argv\n", &all);
	ft_parcer(&all);
	ft_init_plr(all.map, &plr, &all);
	win.mlx = mlx_init();
