...
static	void	ft_plr_check(t_plr *plr, t_all *all)
{
	if (plr->x == -1)
		return ;
	else
...
static	void	ft_plr_plr(char **map, t_plr *plr, t_point pos, t_all *all)
{
	if (ft_strchr("N", map[pos.y][pos.x]))
	{
		ft_plr_check(plr, all);
		set_plr_plr(plr, pos);
		plr->dir = 1 * M_PI_2;
	}
